...
load("./simulation/run methods/Results.dec.muraro.all.RData")
plot.list.muraro <- list()
library(philentropy)
library(cowplot)
niter <- 10
R_square_muraro = matrix(NA, nrow = niter, ncol = length(method_index))
RMSE_muraro = matrix(NA,     nrow =niter, ncol = length(method_index))
cell_type_muraro <- sim_pbac_seg_muraro_wang$ref_batch_muraro$cell_type
for (k in 1:length(method_index)) {
  for (i in 1:niter) {
    temp1 <- Results.dec.muraro[[i]][[1]][[k]][,cell_type_muraro]
    temp2 <- sim_pbac_seg_muraro_wang$spot_data[[i]]$sim_spots_pro[,cell_type_muraro]
    R_square_muraro[i, k] <- cor(c(temp1), c(temp2), method = "pearson")
    RMSE_muraro[i, k] <- sqrt(mean((temp2- temp1)^2))
  }
}
colnames(R_square_muraro) = method_names
colnames(RMSE_muraro) = method_names
sort(apply(R_square_muraro,2, median))
sort(apply(RMSE_muraro,2, median))

####barplot in R

df1_box <- data.frame(PCC = c(R_square_muraro),
                      RMSE = c(RMSE_muraro),
...
# error_box
plot.list.muraro[[1]] <- error_box

RMSE_box = ggplot(df1_box, aes(x = Method , y = RMSE,
                               fill = Method))+
  geom_boxplot()+
  theme_classic()+
  labs(y="RMSE",x=" ", title = "Muraro") +
  theme(
    axis.text.x.bottom = element_blank(),
    axis.text.y.left = element_text(size = 12),
    axis.title.x = element_blank(),
...
plot.list.muraro[[2]] <- RMSE_box
## plot the JSD
library(philentropy)
# source("get_jsd.R")
source("./simulation/plot/get_jsd.R")
nspot <- nrow(Results.dec.muraro[[1]][[1]][[1]])
...
# jsd.ggplot
plot.list.muraro[[3]] <- jsd.ggplot
names(plot.list.muraro) <- c("muraro_PCC", "muraro_RMSE", "muraro_JSD")
# save(plot.list.muraro, file = "ggplot.muraro.evaluation.RData")

pfig2_temp <- c(plot.list.muraro[1], plot.list.muraro[2], plot.list.muraro[3])
pfig2 <- cowplot::plot_grid(  plotlist = pfig2_temp, labels = letters[1:3], nrow = 3, label_size = 20)
print(pfig2)
# ggsave(pfig2, filename = "evaluation.muraro.pdf", width = 12, height = 14)
# ggsave(pfig2, filename = "evaluation.muraro.eps", width = 12, height = 14)

###### plot dot
w.mean <- Reduce("+", list_w_ensemble_muraro)/length(list_w_ensemble_muraro)
correlation.mean <- apply(R_square_muraro,2, mean)[1:length(w.mean)]
...
p2 = ggplot(data_bar, aes(x = PCC , y = weight_est, color = factor(Method))) +
  ...
  labs(y="Weights assigned to \n individual methods",
       x="PCC of individual methods",title = "Muraro") +
  ...
  stat_cor(method='pearson', cor.coef.name = "tau", size = 4,
           label.x = 0.2, label.y = 0.095,color='red') +
  stat_cor(method='spearman', cor.coef.name = 'rho', size = 4,
           label.x = 0.2, label.y = .085,color='red')
print(p2)
# ggsave(p2, filename = "wplot.pdf", width = 6, height = 5)
# ggsave(p2, filename = "wplot.eps", width = 6, height = 5)
#
# plot.list.w.muraro <- p2
# save(plot.list.w.muraro, file = "ggplot.muraro.weight.RData")
